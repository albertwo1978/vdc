...
1. *Azure Key Vault* - A Key Vault is deployed as part of the archetype and is intended to host secrets used to manage the AKS cluster and applications hosted in the cluster. No keys/certs are created in the Key Vault as part of the deployment. 
...
1. *Services requiring additional consideration* The following resources are also deployed as part of the AKS archetype and require additional considerations. These resources are coved in more details in the link:overview.adoc#deployment-assumptions[deployment assumptions] section below: 
   - *Container Registry*
   - *Audit Logging* 
   - *AAD Roles*
   - *Kubernetes personas* 
