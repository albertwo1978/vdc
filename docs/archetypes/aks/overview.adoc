...
1. *Kubernetes personas* - This deployment integrates kubernetes RBAC with Azure Active Directory and creates a sample set of personas for managing AKS cluster resources. As noted above, you should evaluate your specific requirements and make appropriate changes. A description of those personas and their permission levels can be found below:
...
Modifications to these `archetypes.json` parameters are required in order to successfully deploy the archetype. The following table describes the minimum modifications required to deploy under default conditions. Additional modifications can be made, but are not covered in this article. 
...
*STOP HERE* - Need a link to prevent user from deploying next section. 
...
==== Step 5: Deploy AKS workload resources
