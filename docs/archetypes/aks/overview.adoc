...
| Primary resource group for AKS cluster resources.
|===

[ALBERTWO REVIEW - Need link / comment] *STOP HERE* - Need a link to prevent user from deploying next section. 
...
==== Step 5: Deploy AKS workload resources - Required role: ??? [CB REVIEW - Account needs Sub Owner permissions - ANNA FEEDBACK - Can lesser permissions be used?]
...
These deployments creates the `{org}-{deployment-name}-rg` resource group that hosts the resources described in the following table.

[options="header",cols="a,,a"]
|===
| Resource | Type | Description

[ALBERTWO REVIEW - Need to finish this table]
| `resource1`
| resource1
| Description1.
